...
(function () {
  ...
  CircleLayout.prototype.resetConfig = function (layoutConfig) {
    ...
    this.center = layoutConfig['center'] || [0, 0];
  ...
  CircleLayout.prototype.initAlgo = function () {
    ...
    if (!_self.boolfixeddiameter) {
      for (var i = 0; i < nodeCount; i++) {
        var n = nodes[i];
        tempcirc += n.scaleX * n.radius * 2 * 1.2;
      }
    ...
    for (var i = 0; i < nodeCount; i++) {
      var n = nodes[i];
      if (!_self.boolfixeddiameter) {
        noderadius = n.scaleX * n.radius * 2;
  ...
  CircleLayout.prototype.cartCoors = function (radius, whichInt, theta) {
    var coOrds = [];
    coOrds[0] = this.center[0] + radius * Math.cos(theta * whichInt);
    coOrds[1] = this.center[1] + radius * Math.sin(theta * whichInt);
    return coOrds;
  };
  ...
  GirdLayout.prototype.resetConfig = function (layoutConfig) {
    if (layoutConfig) {
      ...
      this.center = layoutConfig['center'] || [100, 100];
      this.initAlgo();
    }
  };
  ...
  GirdLayout.prototype.initAlgo = function () {
    ...
    for (var i = 0; i < xGridScales; i++) {
      for (var j = 0; j < yGridScales; j++) {
        ...
        tempX += _self.center[0] - xGridScales / 2 * _self.horizontalScale;
        tempY += _self.center[1] + yGridScales / 2 * _self.verticalScale - _self.verticalScale;
        var node = nodes[k++];
        var posData = _self.newLayoutData();
  ...
  KKLayout.prototype.shortPath = function (nodeCount) {
    ...
    _self.links.forEach(function (link) {
      var i = _self.nodeIds.indexOf(link.source);
      var j = _self.nodeIds.indexOf(link.target);
	  console.log(_self.nodeIds);
      dij[i][j] = 1;
      dij[j][i] = 1;
    });
  ...
  ForceD3Layout.prototype.resetConfig = function (layoutConfig) {
    if (layoutConfig) {
      ...
      this.noverlap = layoutConfig['noverlap'] || 'false';
      this.size = layoutConfig['size'] || [1400, 700];
      this.initAlgo();
    }
  };
  ...
  var ForceDirected = function (_nodes, _links) {
    ...
    this.resetConfig = function (layoutConfig) {
      if (layoutConfig) {
        ...
        var size = layoutConfig['size'] || [1200, 800];
        this.width = size[0];
        this.height = size[1];
        that.initAlgo();
      }
  ...
  HierarchicalLayout.prototype.initAlgo = function () {
    ...
    self.nodes.forEach(function (node) {
      ...
      posData.finishx = x;
      posData.finishy = y;
      posData.xdistance = 0.02 * (x - node.x);
      posData.ydistance = 0.02 * (y - node.y);
      node.layoutData = posData;
    });
    self.inited = true;
  };
  HierarchicalLayout.prototype.setData = function (selectedNodeId) {
    ...
    if (self.sortMethod == 'selected') {
      ...
      var _selectedNodeIds = selectedNodes.map(n => {
        return String(n.id);
      });
  ...
  HierarchicalLayout.prototype.getDistribution = function () {
    ...
    self.nodeIdList.forEach(function (nodeId) {
      var node = self.nodesIdMap[nodeId];
      var level = self.hierarchicalLevels[nodeId] == null ? 1 : self.hierarchicalLevels[nodeId];
      if (self.direction == 'UD' || self.direction == 'DU') {
        node.y = self.layerDistance * level;
      } else {
        node.x = self.layerDistance * level;
      }
      var temp = distribution[level];
      if (temp == null) {
        temp = [];
      }
      temp.push(node);
      distribution[level] = temp;
    });
    return distribution;
  };
  ...
  HierarchicalLayout.prototype.crawler = function (progress, node, callbackFlag, _treeIndex) {
    ...
    if (progress[node.id] == null) {
      ...
      for (var i = 0; i < edgesIdsLength; i++) {
        ...
        if (childNode != null && node.id != childNode.id) {
  ...
  HierarchicalLayout.prototype.levelByDirection = function (source, nodeB, edge) {
    var self = this;
    if (self.hierarchicalLevels[source.id] == null) {
      self.hierarchicalLevels[source.id] = 1;
    }
    if (self.hierarchicalLevels[nodeB.id] == null) {
      self.hierarchicalLevels[nodeB.id] = self.hierarchicalLevels[source.id] + 1;
    }
  ...
  HierarchicalLayout.prototype.getPositionForHierarchy = function (node) {
    if (this.direction == 'UD' || this.direction == 'DU') {
      return node.x;
    }
    return node.y;
  };
  ...
  HierarchicalLayout.prototype.shiftBlock = function (parentId, diff) {
    var self = this;
    if (self.direction == 'UD' || self.direction == 'DU') {
      self.nodesIdMap[parentId].x += diff;
    } else {
      self.nodesIdMap[parentId].y += diff;
    }
  };
  ...
  HierarchicalLayout.prototype.validataPositionAndContinue = function (node, nodeId, pos) {
    var self = this;
    if (self.lastNodeOnLevel[nodeId] != undefined) {
      var previousPos = self.getPositionForHierarchy(self.nodesIdMap[self.lastNodeOnLevel[nodeId]]);
      if (pos - previousPos < self.nodeDistance) {
        var diff = previousPos + self.nodeDistance - pos;
        self.shiftBlock(node.id, diff);
      }
    }
    self.lastNodeOnLevel[nodeId] = node.id;
    self.positionedNodes[node.id] = true;
    self.placeBranchNodes(node, nodeId);
  };
  ...
  AvoidOverlapLayout.prototype.getConfig = function () {
    ...
    return [{
      'label': '间距',
      'maxPadding': self.maxPadding
    }];
  };
  AvoidOverlapLayout.prototype.resetConfig = function (layoutConfig) {
    this.initAlgo();
    if (layoutConfig) {
      this.maxPadding = Number(layoutConfig['maxPadding']) || 2;
  ...
  AvoidOverlapLayout.prototype.goAlgo = function () {
    var self = this;
    if (this.inited) {
      if (this.maxIterations++ < this.nodes.length / 4) {
        var q = this.quadtree(this.nodes);
        self.nodes.forEach(function (_node) {
          q.visit(collide(_node));
        });
      }
    }
    function collide(node) {
      var nr = node.radius * node.scaleX * 2 + self.maxPadding,
        nx1 = node.x - nr,
        nx2 = node.x + nr,
        ny1 = node.y - nr,
        ny2 = node.y + nr;
  ...
  AvoidOverlapLayout.prototype.quadtree = function (points) {
    var p,
      i = -1,
      n = points.length;
    if (n && isNaN(points[0].x)) {
  ...
  var SpiralLayout = function SpiralLayout(nodes) {
    this.nodes = nodes;
    this.radius = NaN;
    this.spacing = 10;
    this.clockwise = true;
  ...
  SpiralLayout.prototype.getConfig = function () {
    var self = this;
    return [{
      label: '间距',
      spacing: 10
  ...
  SpiralLayout.prototype.resetConfig = function (layoutConfig) {
    if (layoutConfig) {
      this.spacing = Number(layoutConfig['spacing']) || 10;
      this.center = layoutConfig['center'] || [100, 100];
    }
    this.initAlgo();
  };
  SpiralLayout.prototype.initAlgo = function () {
    var self = this;
    var originx = self.center[0];
    var originy = self.center[1];
    var nodes = self.nodes.sort(function (n1, n2) {
      var x = (n1.inLinks || []).length + (n1.outLinks || []).length;
      var y = (n2.inLinks || []).length + (n2.outLinks || []).length;
      if (x > y) {
        return -1;
    ...
    var root = nodes[0];
    var space = self.spacing;
    var cw = self.clockwise ? 1 : -1;
    var rad = self.radius;
    if (rad <= 0 || isNaN(rad) || !isFinite(rad)) {
      rad = this.diameter(root);
    }
    ...
    for (var i = 1; i < nodeLength; i++) {
      ...
      vert.x = x + originx;
      vert.y = y + originy;
      var nextvert = nodes[i + 1];
      var dia = self.diameter(vert) / 2 + self.diameter(nextvert) / 2;
  ...
  SpiralLayout.prototype.diameter = function (v) {
    ...
    return Math.sqrt(v.radius * v.radius + v.radius * v.radius);
  };
  SpiralLayout.prototype.runLayout = function () {
    if (this.inited) {
      this.goAlgo();
    }
  };
  SpiralLayout.prototype.goAlgo = function () {};
  ...
  layoutFac.prototype.getLayout = function (layoutType, configs) {
    ...
    switch (layoutType) {
      ...
      case "hive":
        ...
        break;
      case "scale":
        layout = new ScaleLayout(nodes, null, 1);
        break;
      case "rotate":
        layout = new RotateLayout(nodes, null, 0);
        break;
      case "grid":
        layout = new GirdLayout(nodes);
        break;
      case "gather":
        layout = new TypeGatherLayout(nodes, links);
        break;
      case "spiral":
        layout = new SpiralLayout(nodes);
        break;
      default:
        break;
    }
